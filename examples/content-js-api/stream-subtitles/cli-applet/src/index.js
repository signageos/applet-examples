...
sos.onReady().then(async function () {
	...
	// https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#event-ontrackschanged
	sos.stream.onTracksChanged((tracks) => {
		console.log('onTracksChanged', tracks);
	});

	// Prepare stream for playback with some additional options (https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#prepare)
	...
	// Play stream forever with some additional options (https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#play)
	...
	// Get all tracks of the selected stream (https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#gettracks)
	const trackInfo = await sos.stream.getTracks(streamId);
	console.log('trackInfo', trackInfo);

	// Select subtitles, audio or video track (https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#selecttrack)
	await sos.stream.selectTrack(streamId, 'TEXT', '0', 1);
	await sos.stream.selectTrack(streamId, 'AUDIO', '2', 3);

	// Reset back to default track (https://sdk.docs.signageos.io/api/js/content/latest/js-video-stream#resettrack)

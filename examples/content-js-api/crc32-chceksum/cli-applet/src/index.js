...
sos.onReady().then(async function () {
	...
	const videoHash = await sos.offline.cache.getChecksumFile('video-1', 'crc32');
	console.log('Video hash: ' + videoHash);
	contentElement.innerHTML += 'Video hash: ' + videoHash + '\n';

	// Generate hash for image
	const imageHash = await sos.offline.cache.getChecksumFile('image-1', 'crc32');
	console.log('Image hash: ' + imageHash);
	contentElement.innerHTML += 'Image hash: ' + imageHash + '\n';

	// Validate hash for video
	const validHashVideo1 = await sos.offline.cache.validateChecksumFile('video-1', expectedVideoHash, 'crc32');
	...
	const validHashImage1 = await sos.offline.cache.validateChecksumFile('image-1', 'incorrect-hash', 'crc32');
